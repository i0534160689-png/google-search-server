...
// 🔎 נתיב לקבלת קריאה ממודול תור
app.post('/google_search', async (req, res) => {
  try {
    const { caller, digits } = req.body; // הנתונים שנשלחים ממערכת ימות

    const query = digits || "ימות המשיח";

    // קריאת API של גוגל (צריך לשים API KEY אמיתי)
    ...
    res.json({
      success: true,
      message: `שלום ${caller}, התוצאה הראשונה היא: ${result}`
    });
  } catch (error) {
    console.error(error);
